...
최근에 JMeter, nGrinder등 성능 측정, 도식화 툴을 알게되어 이러한 도구들을 쓸 수도 있을 것 같다.
...
## 다량의 데이터 삽입 시 JdbcTemplate을 활용한 데이터베이스 커넥션 부족 문제 해결
...
이 문제를 해결하기 위해 커넥션을 최소화하여 데이터를 삽입할 수 있는 방법을 알아보기 시작했다.
...
`saveAll()` 메서드의 구현 부분을 보면 하나의 트랜잭션에서 다수의 데이터를 삽입하는 로직으로 구성되어있다.

실제로 테스트를 통해 반복문 내부에 `save()`메서드를 호출하는 것과 `saveAll()`메서드를 호출하는 것을 측정했을 때

`save()`메서드는 6134ms

`save()` 메서드는 4736ms가 소요되었다.

로직을 수행하는 시간 자체가 줄었으니 커넥션을 가지고 있는 갯수도 줄었음을 확인할 수 있었다.
...
## AOP, ThreadLocal을 활용하여 사용자 인증/인가 로직 간소화
