...
또한 Passport는 오직 마이크로서비스간 통신할 때만 사용된다. 즉, 외부로 절대 노출되지 않는 토큰으로 외부 클라이언트에게 제공함으로써 네트워크상에 노출되는 JWT와는 보안성 측면으로도 강점이 있다.
...
3. AuthMicroService가 JWT를 통해 추출한 유저 정보를 바탕으로 Passport를 생성한다.
