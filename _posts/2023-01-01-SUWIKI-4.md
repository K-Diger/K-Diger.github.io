...
graph TD
    ...
    classDef cacheBox fill:#e1f5fe,stroke:#01579b,stroke-width:2px
...
graph TD
    ...
    classDef flowBox fill:#f3e5f5,stroke:#4a148c,stroke-width:2px
...
graph TD
    subgraph Terms["주요 용어"]
        T1["Candidate: Window/Protected Cache에서<br/>Probation Cache로 진입을 시도하는 데이터"]
        T2["Victim: Probation Cache 내에서<br/>제거 대상이 될 수 있는 데이터"]
        T3["Window Cache: 버스트 트래픽으로부터<br/>캐시 오염을 방지하는 첫 번째 방어선"]
        T4["Probation Cache: 데이터의<br/>유용성을 검증하는 공간"]
        T5["Protected Cache: 자주 사용되는<br/>데이터를 보관하는 공간"]
    end

    classDef termBox fill:#f1f8e9,stroke:#33691e,stroke-width:2px
