...
graph LR
   subgraph CaffeineStructure["Caffeine Cache"]
       direction LR
       NewData[새로운 데이터]
       WindowCache[Window Cache<br/><br/>전체 크기의 1%]
       WE[방출된 데이터]
       ProbationCache[Probation Cache<br/><br/> Main Cache 20%]
       ProtectedCache[Protected Cache<br/><br/> Main Cache 80%]
       PE[방출된 데이터]
   ...
   classDef cacheBox fill:#2a4858,stroke:#68c1e8,stroke-width:2px,color:#fff
   class WindowCache,ProbationCache,ProtectedCache cacheBox
   style CaffeineStructure fill:#2a2a2a,stroke:#666,stroke-width:2px,color:#fff
```
