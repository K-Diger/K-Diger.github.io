...
GC의 대상이 되기 위해선 객체를 Unreachable Reference로 만들어줘야한다. 코드 단에서 이를 만족시킬 부분이 있는지 분석하기 시작했다.

`문제점 1`에서 해결한 벌크 삽입 코드를 보았을 때 이론적으로 800 ~ 1,500개의 객체를 데이터베이스에 삽입 직전까지 들고있는 것이 최적화 지점이라고 생각했다.
...
하지만 여전히 간헐적으로 OOM문제가 발생하여 최후의 보루를 선택했다.
