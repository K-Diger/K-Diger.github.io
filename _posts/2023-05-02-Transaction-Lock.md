...
락과 트랜잭션은 유사해 보이지만 다르다.

락은 동시성을 제어하기 위한 기능이고, 트랜잭션은 데이터의 정합성을 보장하기 위한 기능이다.

### 락이 없다면?

하나의 회원 정보 레코드를 여러 커넥션에서 동시에 변경할 때 락이 없다면 여러 커넥션에서 동시에 변경하게 된다.

따라서 데이터가 어떻게 변경되었는지 각 커넥션 마다 보장할 수 없다.
...
## InnoDB와 MyISAM의 트랜잭션 차이점 p.156

- AUTO_COMMIT을 사용한다고 가정

MyISAM 엔진은 데이터 변경 중 오류가 발생했지만 데이터가 롤백되지 않았고

InnoDB는 예상했던 대로 롤백이 되어있었다.

Memory 스토리지 엔진을 사용하는 테이블도 MyISAM과 같은 결과를 가진다.
...
트랜잭션을 개선 전 처럼 하지 않는게 왜 좋냐면, 일단 커넥션을 너무 오래 잡고 있는것이다.

또한 메일전송 FTP 파일 전송 작업 등 네트워크를 통해 다른 서버와 통신하는 작업은 DBMS 트랜잭션과 엮이지 않는게 좋다.

프로그램이 실행되는 동안 외부 통신을 하는 과정에서 문제가 발생하면 DBMS까지 그 영향이 끼치게 된다.

그리고 조회만 하는 내용은 굳이 트랜잭션을 걸지 않아도 상관없다. `트랜잭션 == 락` 이라는 사고방식을 버린다면 이 이유도 당연하다.
...
만약 인덱스가 없다면 테이블을 풀 스캔하면서 UPDATE작업을 수행하게 되는 것이다.
...
## Read UnCommitted (커밋, 롤백여부에 상관없이 다 읽어가십쇼)

- Dirty Read 발생
- Non-Repeatable Read 발생
- Phantom Read 발생

근데 이 격리 수준은 심각한 문제가 있다. 아래의 시나리오를 보자

1. 유저 A가 INSERT INTO user VALUES("KIM", 50000) 이라는 쿼리를 보냈다.
2. 유저 B가 SELECT * FROM USER WHERE id = 50000 이라는 쿼리를 보냈다. (단, 위 유저 A가 보낸 쿼리는 커밋되지 않았다.)
   3. 유저 B는 이 내용을 가지고 추가적인 작업을한다.
4. 유저 A의 INSERT 구문이 롤백되었다. 즉, 실제 테이블에 INSERT되지 않은 채로 트랜잭션이 종료되었다.
...
또한, Dirty Read는 READ UNCOMMITTED에서만 발생하며 이 격리 수준은 RDBMS 표준에서도 눈길도 안주는 수준이다.

---

## Read Committed(커밋, 롤백이 완료된 것만 읽어 가십쇼)
...
1. 유저 A가 SELECT * FROM USER WHERE id = 50000 이라는 쿼리를 보냈다. 이 때 아무것도 조회되지 않았고, 트랜잭션은 종료되지 않았다.
2. 유저 B가 UPDATE user SET(id) = 50000 WHERE ID = 50001; 이라는 쿼리를 보낸 후 커밋을 완료했다.
3. 유저 A가 SELECT * FROM USER WHERE id = 50000 이라는 쿼리를 보냈다. 이 때 한 레코드가 조회가 되었다.
...
## Repeatable Read(커밋, 롤백이 완료된 것만 읽어 가십쇼)
...
이 변경 방식을 MVCC 라고 하는데 앞서 보았던 Read Committed 방식도 이와 같은 방식이긴하다.

이 둘의 차이점은 Repeatable Read 방식은 Undo영역에 백업된 레코드의 여러 버전 중 어떤 것을 사용할 것이냐이다.
...
1. 사용자 B가 트랜잭션 번호 10으로 SELECT * FROM user WHERE id = 50000 쿼리를 날렸다.
2. 사용자 A가 트랜잭션 번호 12로 UPDATE SET first_name = "SONG" WHERE id = 50000 쿼리를 날렸다.
   3. 이 때 Update쿼리를 수행해서 레코드를 변경하기 전에, 기존 레코드 내용을 Undo 로그로 복사한다. (트랜잭션 번호 포함(6))
4. 사용자 A가 날린 트랜잭션 12가 커밋 되었다.
5. 사용자 B가 SELECT * FROM user WHERE id = 50000 쿼리를 또 날렸다.
6. 이 때 결과는 Undo 로그에 저장된 아까 그 내용이 나온다. (트랜잭션 번호가 12가 아닌 6짜리가 나오는 것)
...
0. 트랜잭션 번호 6번으로 테이블의 레코드가 2개 추가되었다.
1. 사용자 B가 트랜잭션 번호 10으로 SELECT * FROM user WHERE id >= 50000 FOR UPDATE 쿼리를 날렸다. (트랜잭션은 아직 살아있다.)
...
`다시 정리하자면 다른 트랜잭션에 의해 변경이 있는 레코드일 경우 SELECT 시 Undo 로그를 읽어와야 하는데 실제 레코드를 읽어오는 Phantom Read 현상으로 인해 non-repeatable이 발생한 것`

InnoDB를 사용하면 스토리지 엔진에서 제공하는 Gap Lock, Next Key Lock 덕분에 Repeatable Read 수준에서도 Phantom Read가 발생하지 않는다.

---

## Serializable(이 트랜잭션에 대한 데이터는 영속화 대상입니다. (직렬화))
