...
flowchart TB
    subgraph G1GC["G1 Garbage Collection Cycle"]
        Start(["Eden 영역<br>할당 임계치 도달"]) --- Y1(["Young GC<br>Eden → Survivor 복사<br>Survivor → Old 승격"]) --- Y2{{"Old Gen<br>점유율 > 45%?"}}
        Y2 --- CS(["Concurrent Start<br>애플리케이션 실행과 동시에<br>Old 영역 객체 표시 시작"])
        Y2 --- Start
        CS --- RM(["Remark (STW)<br>최종 마킹 완료<br>참조 처리 및 클래스 Unloading"])
        RM --- CL(["Cleanup (STW)<br>빈 영역 회수<br>다음 단계 Mixed GC 결정"])
        CL --- MX(["Mixed GC<br>Young 영역 + Old 영역 수집<br>Old 영역의 가비지 비율이<br>5% 이하가 될 때까지 반복"])
        MX --- Start
    end
